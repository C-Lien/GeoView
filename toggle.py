...
class Toggle():
    ...
    def toggle_local_layer_text_single_display(parent, layer_name):

        ObjectIO.clear_view_items(parent, 'show_layer_names')

        for layer_dict in parent.tracker_dict['show_layer_names']:
            if layer_dict['layer_name'] == layer_name:
                layer_dict['show'] = not layer_dict['show']

        if parent.bool_dict['show_layer_names']:
            Text().set_layer_text(parent)

    def toggle_local_layer_text_all_display(parent):

        if not parent.bool_dict['show_layer_names']:
            Text().set_layer_text(parent)
        else:
            ObjectIO.clear_view_items(parent, 'show_layer_names')

        parent.bool_dict['show_layer_names'] = not parent.bool_dict['show_layer_names']
    ...
    def toggle_local_triangulations_display(parent):
